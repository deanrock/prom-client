...
class AggregatorRegistry extends Registry {
	...
	setWorkers(workers = []) {
		if (workers.find(worker => !(worker instanceof Worker))) {
			throw new Error('Provided workers must be of type Worker.');
		}
...
function addListeners() {
	...
	if (cluster().isWorker) {
		...
		process.on('message', message => {
			if (message.type === GET_METRICS_REQ) {
				...
				for (const worker of workersQueue) {
	...
	if (!isMainThread) {
		parentPort.on('message', async ({ type, requestId, port } = {}) => {
